...
if(isset($_POST['Login-entry'])) {
  ...
  $sql = "SELECT * FROM  stakeholders WHERE id = '$id' AND last_name = '$lname' AND email = '$email' AND
   contact_No = '$contact' AND status = 'Pending'"; 
  ...
  if($total > 0){
  $_SESSION['ID'] = $row['id'];
  $_SESSION['Login-entry'] = $row['last_name'];
  $_SESSION['email'] = $row['email'];
  $_SESSION['contact_No'] = $row['contact_No'];
  $_SESSION['status'] = $row['status'];
  
  echo "<script type='text/javascript'>alert('Welcome Stakeholder!')
  window.location.href = 'Edit_entry_data.php?ID=$id';

  </script>";
  }else{
    echo "<script type='text/javascript'> alert('Wrong Last name and Email and contact. OR Only Pending Appointment can be Edited') 
    window.location.href = 'Login_entry.php?ID=$id';</script>";
  }
...
<section>
  ...
  <div class="box">
   ...
   <div class="container-login"> 
    ...
    <div class="head">
      <img src="Picture/document.png">
    </div>
    <?php  if(!isset($_POST['Login-entry'])){?>
      <h3 >For Pending appointmetns only</h3> 
      <h3>LOGIN For Details Changes</h3>
     
      <form method="post">

        <div class="inputBx">       
          <h2  style="color:white;">Entry ID: <?php echo $row['id']; ?></h2>
          <input type="text" id="entry" value="<?php echo $row['id'];?>" style="display:none"/> 
        </div>
        ...
        <div class="inputBx password">
          <input type="tel" maxlength="11" id="password-input" name="contact_No"  pattern="[0]{1}[0-9]{10}" required="required">
          <span>Contact</span>
        ...
        <div class="inputBx">
            ...
            <button  type="submit" name="Login-entry" >Login</button>
